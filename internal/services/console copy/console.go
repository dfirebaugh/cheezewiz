...
func (c *console) setDestination() string {
	if !c.input.hasArgs() {
		logrus.Warn("no username set")
		return ""
	}
	if c.input[1] == "" {
		logrus.Warn("no destination was set")
		return ""
	}

	c.destination = c.input[1]
	if c.destination == "world" {
		c.response = fmt.Sprintf("broadcasting to [%s]", c.destination)
		logrus.Info("broadcasting to: ", c.destination)
		return fmt.Sprintf("broadcasting to: [%s]", c.destination)
	}

	c.response = fmt.Sprintf("whispering [%s]", c.destination)
	logrus.Info("setting destination to: ", c.destination)
	return fmt.Sprintf("whispering: [%s]", c.destination)
}
